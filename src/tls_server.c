...
static struct option cli_options[] = {{"port", required_argument, 0, 'p'},
...
void print_help(FILE *fp, const char *progname) {
  ...
  fprintf(fp,
          "                  The port the port should start listening on "
          "(default: %u)\n",
          DEFAULT_PORT);
  fprintf(fp, "-h, --help\n");
...
void configure_context(SSL_CTX *ctx) {
  ...
  if (access("server.crt", F_OK) == 0) {
    ...
    X509_NAME_add_entry_by_txt(name, "C", MBSTRING_ASC, (unsigned char *)"CH",
                               -1, -1, 0);
    X509_NAME_add_entry_by_txt(
        name, "O", MBSTRING_ASC,
        (unsigned char *)"Universita della Svizzera italiana", -1, -1, 0);
...
int main(int argc, char *argv[]) {
  ...
  int opt, opt_index;
  while (optind < argc) {
    if ((opt = getopt_long(argc, argv, "p:h", cli_options, &opt_index)) != -1) {
      switch (opt) {
      case 'p': {
  ...
  while (1) {
    ...
    printf("received %s from %s:%d\n", request,
           inet_ntop(AF_INET, &addr.sin_addr, cli_addr, sizeof(cli_addr)),
           ntohs(addr.sin_port));
